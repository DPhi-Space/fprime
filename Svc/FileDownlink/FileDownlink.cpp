...
namespace Svc {
  ...
  void FileDownlink::SendFileToNode_cmdHandler(
    FwOpcodeType opCode,
    U32 cmdSeq,
  ...
  )
  {
    ...
    entry.destination = destinationNode;
    ...
    )
  {
    ...
    this->m_file.setDestinationNode(dest);
  ...
  {
    const U32 packetSize = filePacket.bufferSize();
    const U32 totalSize = packetSize + sizeof(Components::Node::SerialType);

    FW_ASSERT(this->m_buffer.getData() != nullptr);
    FW_ASSERT(this->m_buffer.getSize() >= totalSize,
      static_cast<FwAssertArgType>(totalSize),
      static_cast<FwAssertArgType>(this->m_buffer.getSize()));

    // Create a data buffer that starts after the space for the destination node
    Fw::Buffer data_buffer(this->m_buffer.getData() + sizeof(Components::Node::SerialType), packetSize);

    // Serialize file packet into data buffer
    Fw::SerializeStatus status = filePacket.toBuffer(data_buffer);
    FW_ASSERT(status == Fw::FW_SERIALIZE_OK);

    // Serialize the destination node at the beginning of m_buffer
    Fw::SerializeBufferBase& serializeRepr = this->m_buffer.getSerializeRepr();
    serializeRepr.resetSer(); // Ensure the buffer is reset before serializing
    Components::Node::T dest = this->m_file.getDestinationNode();
    status = serializeRepr.serialize(dest);
    FW_ASSERT(status == Fw::FW_SERIALIZE_OK);

    // Set the total buffer size to include the destination node and file packet
    this->m_buffer.setSize(totalSize);

    // Send out the buffer
    this->bufferSendOut_out(0, this->m_buffer);

    // Restore buffer size to max
    this->m_buffer.setSize(FILEDOWNLINK_INTERNAL_BUFFER_SIZE + sizeof(Components::Node::SerialType));
    this->m_packetsSent.packetSent();
  }
